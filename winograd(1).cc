...
void image_transform(float *__restrict__ packed_image,
  ...
  for (int64_t i = 0; i < collapsed_dim_size * ti.tile_in_w; ++i) {
    int64_t idx = i / ti.tile_in_w;
    int64_t w = i % ti.tile_in_w;
  ...
  #pragma omp parallel for num_threads(32)
  for (int64_t i = 0; i < collapsed_dim_size * ti.tile_in_h; ++i) {
    int64_t idx = i / ti.tile_in_h;
    int64_t h = i % ti.tile_in_h;
    z6 = V_tensor[h][0][idx];
...
void filter_transform(float *__restrict__ packed_filter,
  ...
  #pragma omp parallel for num_threads(32)
  for (int64_t i = 0; i < collapsed_dim_size * fs.w; ++i) {
    int64_t idx = i / fs.w;
    int64_t w = i % fs.w;
  ...
  #pragma omp parallel for num_threads(32)
  for (int64_t i = 0; i < collapsed_dim_size * us.h; ++i) {
    int64_t idx = i / us.h;
    int64_t h = i % us.h;
    z6 = U_tensor[h][0][idx];
...
void output_transform(float *__restrict__ M,
  ...
  #pragma omp parallel for num_threads(32)
  for (int64_t i = 0; i < collapsed_dim_size * ti.tile_out_h; ++i) {
    int64_t idx = i / ti.tile_out_h;
    int64_t h = i % ti.tile_out_h;
    z4 = Y_tensor[h][0][idx];
...
void filter_packing(float *__restrict__ filter, float *__restrict__ packed_filter, const filter_shape_t fs) {
  ...
  #pragma omp parallel for collapse(4) num_threads(32) schedule(static) 
...
void sgemm(const int64_t M, const int64_t N, const int64_t K, float *A, float *B, float *C) {
  ...
  #pragma omp parallel for collapse(2) num_threads(32) schedule(static)
